...
def run_sbatch_pinn_cross_validation(smooth_temp=False):
    ...
    for genotype in genotype_list:
        ...
        with open(job_file, 'w') as file1:
            ...
            file1.writelines('conda run -n test\n')
            file1.writelines(
                "python3 ODE_neural.py -if_pinn False -genotype {0} -smooth_temp {1}\n"
                .format( genotype,smooth_temp))
            file1.close()
        # os.system("sbatch %s" % job_file)
        os.system("python3 ODE_neural.py -if_pinn False -genotype {0} -smooth_temp {1} &\n"
                .format( genotype,smooth_temp))
        os.system('rm {}'.format(job_file))
        time.sleep(2000)
            ...
            for genotype_encoding in encoding_list:#
                for smooth_input in [smooth_input]:
                    ...
                    with open(job_file, 'w') as file1:
                        ...
                        file1.writelines(
                            "python3 g_e_height.py -mode {0} -if_pinn {1} -smooth_loss {2} -genotype_encoding {3} -split_group {4} -reduce_time_resolution {5} -smooth_input {6} -smooth_temp {7}\n"
                            .format(job_file, if_pinn,smooth_loss,genotype_encoding,split_group,reduce_time_resolution,smooth_input,smooth_temp))
                    file1.close()
                    print("python3 g_e_height.py -mode {0} -if_pinn {1} -smooth_loss {2} -genotype_encoding {3} -split_group {4} -reduce_time_resolution {5} -smooth_input {6} -smooth_temp {7}\n"
                            .format(job_file, if_pinn,smooth_loss,genotype_encoding,split_group,reduce_time_resolution,smooth_input,smooth_temp))
                    os.system("sbatch %s" % job_file)
                    #os.system("python3 g_e_height.py -mode {0} -if_pinn {1} -smooth_loss {2} -genotype_encoding {3} -split_group {4} -reduce_time_resolution {5} -smooth_input {6} -smooth_temp {7}\n"
                    #       .format(job_file, if_pinn,smooth_loss,genotype_encoding,split_group,reduce_time_resolution,smooth_input,smooth_temp))
                    # time.sleep(7200)
    else:
